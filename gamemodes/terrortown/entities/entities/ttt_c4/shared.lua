   ...
   local d = 0.0
      ...
      for _, player in ipairs(player.GetAll()) do
         if IsValid(player) and player:Team() == TEAM_TERROR then
            ...
            diff = player_pos - center
            distance = math.sqrt(diff:Dot(diff))

            print("Bomb position", center)
            print("Player Position = ", player_pos)
            print("Distance between Player and Bomb = ", distance)

            if distance > (DEADLY_RADIUS + DAMAGE_RADIUS) then return end

            if distance > DEADLY_RADIUS then

               --If the C4 is behind a wall, then apply additional falloff
               local traceResult = util.QuickTrace(center, diff, player)

               -- Check if trace hit the world
               if traceResult.HitWorld then
                   dmg_reduction = (100 * math.ease.OutExpo( (distance - 540) / 460))
               else
                   dmg_reduction = (100 * math.ease.OutQuad( (distance - 540) / 460))
               end
...
function ENT:Explode(tr)
   ...
   if SERVER then
      ...
      local adjusted_damage_radius = DEADLY_RADIUS + DAMAGE_RADIUS
      local adjusted_effect_radius = EFFECT_RADIUS

      if self.DisarmCausedExplosion then
         adjusted_damage_radius = ADJUSTED_DAMAGE_RADIUS / 2.5
         adjusted_effect_radius = adjusted_effect_radius / 2.5
      end

      -- damage through walls
      self:SphereDamage(dmgowner, pos, adjusted_damage_radius)

      local effect = EffectData()
      effect:SetStart(pos)
      effect:SetOrigin(pos)
      -- these don't have much effect with the default Explosion
      effect:SetScale(adjusted_effect_radius)
      effect:SetRadius(adjusted_effect_radius)
      effect:SetMagnitude(EFFECT_MAGNITUDE)
      ...
      phexp:SetPos(pos)
      phexp:SetKeyValue("magnitude", EFFECT_MAGNITUDE)
      phexp:SetKeyValue("radius", adjusted_effect_radius)
